    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                pthread_rwlock_wrlock(&inode_table[inumber].rwl);
                ...
                pthread_rwlock_unlock(&inode_table[inumber].rwl);
            }
            return inumber;
        }
    }
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_node_type == T_FILE) {
        pthread_rwlock_wrlock(&inode_table[inumber].rwl);
        for (int i = 0; i < ceil((double)inode_table[inumber].i_size / BLOCK_SIZE); i++) {
            if (i < BLOCKS_PER_FILE){
                ...
                void *block_sup = data_block_get(inode_table[inumber].i_sup_block);
        ...
        if(inode_table[inumber].i_size > BLOCK_SIZE * BLOCKS_PER_FILE){
            if (data_block_free(inode_table[inumber].i_sup_block) == -1) {
                pthread_rwlock_unlock(&inode_table[inumber].rwl);
                return -1;
            }
        }
...
int find_in_dir(int inumber, char const *sub_name) {
    insert_delay(); // simulate storage access delay to i-node with inumber
    if (!valid_inumber(inumber) || inode_table[inumber].i_node_type != T_DIRECTORY) {
        return -1;
    }
